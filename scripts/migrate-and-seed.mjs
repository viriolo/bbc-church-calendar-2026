...
async function migrate() {
  ...
  `;
  console.log('✓ quarters table created');
  ...
  // Create index on events.date for calendar queries
  await sql`
...
async function seed() {
  ...
  const existingQuarters = await sql`SELECT COUNT(*) as count FROM quarters`;
  if (Number(existingQuarters[0].count) > 0) {
  ...
  `;
  console.log('✓ events seeded (58 events)');
